...
impl Passport {
    fn from_string(string: String, pattern: &Regex) -> Passport {
        let map: HashMap<String, String> = pattern.captures_iter(&string)
    ...
    fn is_valid(&self) -> bool {
        self.has_all_needed() && self.map.iter()
            .filter(|(k, v)| match k.as_str() {
                ...
                "hgt" => match (&v[..v.len() - 2].parse::<u32>().unwrap_or(0), &v[v.len() - 2..]) {
                    (i, "cm") => 150 <= *i && *i <= 193,
                    (i, "in") => 59 <= *i && *i <= 76,
                    _ => false
                },
                "hcl" => v.starts_with("#") && Passport::valid_int_range(&v.trim_start_matches("#").to_string(), -1, -1, 6, 16),
                "ecl" => ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].iter()
                    .filter(|x| x == v)
                    .count() != 0,
                "pid" => Passport::valid_int_range(v, -1, -1, 9, 10),
                "cid" => true,
                _ => {println!("wut: {:?}", k); false}
            }).count() == self.map.len()
    }
    ...
    fn load_passports(fpath: String) -> io::Result<Vec<Passport>> {
        ...
        let blocks: Vec<Passport> = read_to_string(fpath)?.split("\n\n")
            .map(|s| s.to_string())
...
fn main() {
    let fpath = env::args().nth(1)
                           .unwrap_or("input.txt".to_string());
    let passports = Passport::load_passports(fpath).unwrap();

    let valid1_cnt = passports.iter()
        .filter(|p| p.has_all_needed())
        .count();
    let valid2_cnt = passports.iter()
        .filter(|p| p.is_valid())
        .count();
    println!("Day4:\n  Part1: {}\n  Part2: {}", 
        valid1_cnt,
